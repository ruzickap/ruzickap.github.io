...
while read -r GITHUB_REPOSITORY_TITLE_TMP; do
  ...
  # Keep only real workflows defined in .github/workflows; this drops GitHub-managed
  # "dynamic/*" entries (pages-build-deployment, Dependabot Updates, Copilot agents, ...)
  GITHUB_REPOSITORY_CI_CD_STATUS=$(curl -s --header "authorization: Bearer ${GITHUB_TOKEN}" "https://api.github.com/repos/${GITHUB_REPOSITORY_NAME}/actions/workflows" | jq -r '.workflows[] | select(.path | startswith(".github/")) | "  [![GitHub Actions status - " + .name + "](" + .badge_url + ")](" + .html_url | gsub("/blob/.*/.github/"; "/actions/") + ")"' | sort --ignore-case)
