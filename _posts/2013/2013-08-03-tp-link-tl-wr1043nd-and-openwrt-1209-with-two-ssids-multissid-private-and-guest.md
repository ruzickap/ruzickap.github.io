...
![image](https://rawgithub.com/ruzickap/linux.xvx.cz/gh-pages/pics/openwrt/wifi_openwrt2.svg)
...
Configure the wifi_priv - private wifi
...
rm /etc/resolv.conf
cat > /etc/resolv.conf /etc/skel/.mc/ini /usr/bin/mc-wrapper.sh
chmod a+x /usr/bin/mc-wrapper.sh

echo "ruzickap ALL=(ALL) NOPASSWD:ALL" >> /etc/sudoers

cat >> /etc/screenrc > /etc/profile /etc/rsyncd.conf > /etc/sysctl.conf
```
...
cat > /etc/digitemp.script /www/index.html
...
cat >> /etc/lighttpd/lighttpd.conf "/usr/bin/lua" )
...
cat > /etc/graphs-vnstat.sh > /etc/crontabs/root

cat > /www/myadmin/vnstat/index.html

 Traffic of OpenWRT interfaces
...
cat >> /www/myadmin/vnstat/index.html
 ...
 ![$IFCE Summary](vnstat_${IFCE}_s.png)

 ![$IFCE Hourly](vnstat_${IFCE}_h.png)

 ![$IFCE Daily](vnstat_${IFCE}_d.png)

 ![$IFCE Top 10](vnstat_${IFCE}_t.png)

 ![$IFCE Monthly](vnstat_${IFCE}_m.png)
...
cat > /etc/nodogsplash/nodogsplash.conf /a\
\ \

### For Internet access - click the sign.

### Pro pristup na Internet klikni na znak.
