---
title: TrueNAS Community Edition 25.04 with Plex and qBittorrent
author: Petr Ruzicka
date: 2025-02-18
...
with the latest TrueNAS Community Edition 25.04. The machine is equipped with
2 CPUs, 8GB of RAM and a 64GB eMMC storage.
...
* Shell
  * Run commands:
    ...
    ```bash
    sed -i.orig -e 's/-n3:0:0/-n3:0:+16G/g' /usr/lib/python3/dist-packages/truenas_installer/install.py
    exit
    ```
...
graph LR
    ...
    subgraph shell_dashboard[Shell]
      shell --> commands[sudo su<br>sgdisk -n0:0:0 -t0:BF01 /dev/mmcblk0<br>zpool create -O compression=lz4 -O atime=off my-local-disk-pool /dev/mmcblk0p4<br>zpool export my-local-disk-pool]
    end
...
graph LR
    ...
    subgraph datasets_dashboard[Datasets]
      ...
      dataset_name --> permissions[Permissions]
      permissions --> edit[Edit]
    end
    subgraph edit_acl_dashboard[Edit ACL]
      edit --> add_item["\+ Add Item"]
...
### [Set Up Shares](https://www.truenas.com/docs/scale/25.04/scaleuireference/shares/)
...
graph LR
    ...
    subgraph shares_dashboard[Shares]
      shares --> windows_smb_shares_add["Windows (SMB) Shares -> Add"]
      windows_smb_shares_add --> path[Path: /mnt/my-pool/data]
    end
    click shares "https://truenas.local/ui/sharing"
```
...
graph LR
    ...
    subgraph applications_installed_dashboard[Applications Installed]
      ...
      install --> plex_configuration[Plex Configuration]
      install --> network_configuration[Network Configuration]
      network_configuration --> host_network[Host Network]
      install --> storage_configuration[Storage Configuration]
...
graph LR
    ...
    subgraph data_protection_dashboard[Data Protection]
      ...
      provider --> oauth_authentication[OAuth Authentication -> Logged In Provider]
      provider --> drives_list[Drives List -> OneDrive]
      cloud_sync_tasks --> what_and_when_direction[Direction: PUSH]
      cloud_sync_tasks --> what_and_when_directory_files[Directory/Files: /mnt/my-local-disk-pool/plex]
      cloud_sync_tasks --> what_and_when_folder[Folder: /truenas-backup-plex]
      cloud_sync_tasks --> what_and_when_schedule[Schedule: Weekly]
