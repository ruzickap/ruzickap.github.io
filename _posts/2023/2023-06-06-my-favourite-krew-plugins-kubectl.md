---
title: My Favorite Krew Plugins for kubectl
author: Petr Ruzicka
date: 2023-06-06
description: My favorite krew plugins for the kubectl command-line tool
...
I'd like to share a few notes about the kubectl plugins I use, installed via [krew](https://krew.sigs.k8s.io/). This isn't a comprehensive guide to each plugin, but rather a focus on practical examples and screenshots.
...
Here's a list of my favorite Krew and kubectl plugins:

### [cert-manager](https://github.com/cert-manager/cert-manager)

- A kubectl add-on that automates the management and issuance of TLS certificates. It allows direct interaction with cert-manager resources, such as manually renewing Certificate resources.
...
Get details about the current status of a cert-manager Certificate resource, including information on related resources like CertificateRequests or Orders:
...
- A `top`-like utility for your Kubernetes clusters.
...
Shows all deprecated objects in a Kubernetes cluster, allowing the operator to verify them before upgrading:
...
- A kubectl plugin enabling direct connections to AWS EKS cluster nodes managed by Systems Manager. It relies on the local AWS CLI and the session-manager-plugin being installed.
...
Change the active namespace of the current context and list secrets from `cert-manager` without using the `--namespace` or `-n` flag:
...
- This kubectl plugin makes services accessible via their ClusterIP from outside your cluster.
...
Open the Grafana Dashboard URL in your browser:
...
List Kubernetes RBAC Roles/ClusterRoles used by a given User, ServiceAccount, or Group:
...
View resources and capacity of the nodes:
...
- This plugin summarizes Kubernetes RBAC roles for the specified subjects.

[rolesum](https://github.com/Ladicle/kubectl-rolesum) Krew plugin
installation:
...
- Enables multi-pod and container log tailing for Kubernetes.
...
Check logs for all pods in the `cert-manager` namespace in the past 1 hour:
...
There are a few other "kubectl krew plugins" that I've looked at but am not currently using:
...
[unlimited](https://github.com/nilic/kubectl-unlimited), and [whoami](https://github.com/rajatjindal/kubectl-whoami).
