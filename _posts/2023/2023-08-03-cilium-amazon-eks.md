...
with Karpenter and Cilium, along with standard applications.

The Amazon EKS setup should meet these "cheap" requirements:

- Two AZs only - to reduce costs for cross-availability zone traffic
- Spot instances
- A less expensive region - `us-east-1`
- The most price-efficient EC2 instance type, `t4g.medium` (2xCPU, 4GB RAM),
  using [AWS Graviton](https://aws.amazon.com/ec2/graviton/) based on ARM
- Use [Bottlerocket OS](https://github.com/bottlerocket-os/bottlerocket) - a
  small operating system with a minimal CPU/Memory footprint
- Use
...
- [Karpenter](https://karpenter.sh/) - to autoscale with right-sized nodes
  matching pod requirements

The Amazon EKS setup should meet these security requirements:

- Amazon EKS must be
  [encrypted by KMS](https://docs.aws.amazon.com/eks/latest/userguide/enable-kms.html)
- Worker node
  [EBS volumes need to be encrypted](https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/EBSEncryption.html)
...
- Encryption should use [Wireguard](https://en.wikipedia.org/wiki/WireGuard) as
  the "fastest encryption method"
- Use
  [Elastic Network Interface (ENI)](https://docs.cilium.io/en/v1.14/network/concepts/ipam/eni/#aws-eni)
  integration
...
Create a DNS zone for EKS clusters:
...
delegation. This step depends on your domain registrar. I'm using CloudFlare and
...
Create a CloudFormation template containing the
[Route53](https://aws.amazon.com/route53/) zone and
[KMS](https://aws.amazon.com/kms/) key.

Put the new domain `CLUSTER_FQDN` into Route 53 and configure DNS delegation
...
managedNodeGroups:
  ...
  # (Issue: https://github.com/eksctl-io/eksctl/issues/7003)
  - name: mng02-ng
    amiFamily: Bottlerocket
...
if [[ ! -s "${KUBECONFIG}" ]]; then
  if ! eksctl get clusters --name="${CLUSTER_NAME}" &> /dev/null; then
    ...
    # Add roles created by eksctl to the KMS policy to allow
    # aws-ebs-csi-driver to work with encrypted EBS volumes
...
Get the necessary details about VPC, NACLs, and SGs:
...
Fix the "high" rated security issue: "Default security group should have no
rules configured":
...
Install [Cilium](https://cilium.io/) and remove the nodegroup `mng02-ng` used
for the "eksctl karpenter" installation. (It is no longer needed because Cilium
will be installed and taints will be removed):
...
Interface (CSI) Driver provides a
...
Delete the `gp2` StorageClass, because `gp3` will be used instead:
...
Many K8s services/applications can export metrics to Prometheus. That is why
Prometheus should be the "first" application installed on the K8s cluster.

Then you will need some basic tools/integrations, like
[external-dns](https://github.com/kubernetes-sigs/external-dns),
[ingress-nginx](https://kubernetes.github.io/ingress-nginx/),
[cert-manager](https://cert-manager.io/),
[oauth2-proxy](https://oauth2-proxy.github.io/oauth2-proxy/), etc.
...
Change the [karpenter](https://karpenter.sh/) default installation by upgrading
the [helm chart](https://artifacthub.io/packages/helm/oci-karpenter/karpenter)
and modifying the
...
collect data like metrics and logs from different sources, enrich them with
filters, and send them to multiple destinations.
...
[helm chart](https://artifacthub.io/packages/helm/aws/aws-for-fluent-bit) and
modify the
...
The ServiceAccount `cert-manager` was created by `eksctl`.
...
`external-dns` will take care of DNS records. The ServiceAccount `external-dns`
...
endpoints with Google Authentication.
...
Let's check the Cilium status using the
[cilium command](https://github.com/cilium/cilium-cli):
...
A different way of running `cilium status` on a K8s worker node:
...
Handy details about Cilium networking can be found by listing the `ciliumnodes`
CRD:
...
A handy command to find exposed ports (HostNetwork) to look for port collisions:
...
Remove the EKS cluster and created components:
...
Remove orphan EC2 instances created by Karpenter:
