...
EKS]({% post_url /2022/2022-11-27-cheapest-amazon-eks %}), I'm using
[cert-manager](https://cert-manager.io/) to get the wildcard certificate for the
ingress.

When Let's Encrypt [production](https://letsencrypt.org/about/) certificates are
used, it can be handy to back them up and restore them when the cluster is
recreated.
...
once. The goal is to back up the certificate and then restore it whenever needed
to a "new" cluster.
...
Create a new certificate and let it be signed by Let's Encrypt to validate it:
...
Before installing Velero, it is necessary to create an IRSA with an S3 policy.
The created ServiceAccount `velero` will be specified in the Velero helm chart
later.
...
Install `velero`
[helm chart](https://artifacthub.io/packages/helm/vmware-tanzu/velero) and
modify the
...
serviceAccount:
  server:
    # Use existing IRSA service account
    create: false
    name: velero
credentials:
  useSecret: false
# Create scheduled backup to periodically backup the "production" certificate
# in the "cert-manager" namespace every night:
schedules:
...
The next steps will show how to restore a Let's Encrypt production certificate
(previously backed up by Velero to S3) to a new cluster.
...
`cert-manager/ingress-cert-production`. Let's use this certificate with
`ingress-nginx`.
...
Let's Encrypt certificates are valid for 90 days. It is necessary to renew them
before they expire.

A few commands show the details after cert-manager has renewed the certificate.
